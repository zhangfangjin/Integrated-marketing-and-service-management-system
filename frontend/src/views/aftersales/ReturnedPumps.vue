  ...
  <div class="returned-pumps">
    <el-card>
      ...
      <el-table :data="tableData" v-loading="loading" border style="width: 100%">
        ...
        <el-table-column prop="pumpName" label="返厂泵名称" min-width="150" />
        <el-table-column prop="returnDate" label="返厂时间" width="120" />
        <el-table-column prop="pumpModel" label="泵型号" width="120" />
    ...
    <el-dialog v-model="formVisible" :title="isEdit ? '修改返厂泵信息' : '录入返厂泵信息'" width="500px">
      <el-form :model="form" label-width="100px" :rules="rules" ref="formRef">
        ...
        <el-form-item label="泵名称" prop="pumpName">
          <el-input v-model="form.pumpName" />
        </el-form-item>
        <el-form-item label="返厂时间" prop="returnDate">
          <el-date-picker v-model="form.returnDate" type="date" value-format="YYYY-MM-DD" style="width: 100%" />
        </el-form-item>
        <el-form-item label="泵型号">
          <el-input v-model="form.pumpModel" />
        </el-form-item>
...
const rules = {
  ...
  pumpName: [{ required: true, message: '请输入泵名称', trigger: 'blur' }],
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await api.get('/returned-pumps', { params: { keyword: keyword.value } })
    tableData.value = res
  } finally {
    loading.value = false
  }
...
const saveForm = async () => {
  await formRef.value.validate(async (valid) => {
    if (valid) {
      if (isEdit.value) {
        await api.put(`/returned-pumps/${form.id}`, form)
        ElMessage.success('更新成功')
      } else {
        await api.post('/returned-pumps', form)
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除这条信息吗？', '提示', { type: 'warning' }).then(async () => {
    await api.delete(`/returned-pumps/${row.id}`)
